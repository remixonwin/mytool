...
TRACKER_SCHEMA = {
    ...
    "properties": {
        ...
        "dependencies": {
            ...
            "properties": {"development": {"type": "array", "items": {"type": "string"}}},
        },
        "documentation": {
            "type": "object",
...
def update_tracker(tracker_path: str) -> int:
    ...
    try:
        ...
        dev_deps = [
            ...
            for pkg in packages
            if any(
                tool in pkg.lower() for tool in ["pytest", "black", "ruff", "pre-commit", "pyyaml"]
            )
        ]
        ...
        for src_file in src_files:
            file_info = {
                "path": src_file,
                "status": "Active",
                "last_modified": datetime.fromtimestamp(os.path.getmtime(src_file)).strftime(
                    "%Y-%m-%d"
                ),
            }
        ...
        for test_file in test_files:
            ...
            test_info = {
                "path": test_file,
                "covers": src_file,
                "last_modified": datetime.fromtimestamp(os.path.getmtime(test_file)).strftime(
                    "%Y-%m-%d"
                ),
            }
