...
class TaskManager:
    ...
    async def analyze_task(self, task_description: str) -> Dict[str, Any]:
        ...
        try:
            system_prompt = (
                "You are a project management assistant. Analyze the following task "
                "and provide structured information about its requirements, dependencies, "
            ...
            raise RuntimeError("Failed to analyze task with DeepSeek API") from err
    ...
    async def generate_task_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            system_prompt = (
                "You are a project management assistant. Generate a summary of the tasks, "
                "including progress overview and key metrics."
            )
            ...
            response = await openai.ChatCompletion.acreate(
                ...
                messages=[
                    {"role": "system", "content": system_prompt},
                    {"role": "user", "content": f"Tasks to analyze:\n{task_data}"},
                ],
            )
            ...
            raise RuntimeError("Failed to generate task summary") from err
